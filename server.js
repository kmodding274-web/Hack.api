...
// 1. Server start hote hi pehli baar download hoga
downloadTokenFromServer1();

// 2. Har 30 ghante ke baad automatic download / update hone ka timer
const THIRTY_HOURS = 30 * 60 * 60 * 1000; 

setInterval(() => {
    console.log("[Server 2 Interval] 30 ghante poore ho gaye hain, Server 1 se nayi file download kar rahe hain...");
    downloadTokenFromServer1();
}, THIRTY_HOURS);
...
app.listen(PORT, () => {
    console.log(`Server 2 hardcore mode + 30 Hours Auto-Update ke sath port ${PORT} par chal raha hai!`);
});
